...
## CHECK_8_VOICE_PROFILE_ALIGNMENT (Centrifuge)

```
ADDED: VFP-05 (2026-07-16)
SOURCE: core/centrifuge.py — quantitative voice profile scoring
DATA: voice.db register_profiles + rarity_targets (populated from VFP-04 analysis of 42,363 messages)

SCORING:
  Input: generated text + target register (mapped from environment)
  Output: overall_score (0-1), sub-scores (rarity, rhythm, style), flags

  PASS:  overall_score >= 0.65
  WARN:  overall_score >= 0.50 but < 0.65 (passes but review flags)
  FAIL:  overall_score < 0.50 (rewrite required)

SUB-SCORES:
  Rarity (weight 0.4): vocabulary frequency alignment with David's kite shape
  Rhythm (weight 0.3): sentence length distribution alignment
  Style  (weight 0.3): contraction/punctuation/emphasis alignment

FLAGS (actionable items):
  em_dash:              Any em-dash or en-dash found - replace with hyphen
  low_rarity:           Words too common (zipf > 5.5) for register
  sentence_too_uniform: Sentence length std < 3.0 in a bimodal register
  claude_vocabulary:    Known Claude-tell words (utilize, facilitate, leverage, etc.)

ENVIRONMENT-TO-REGISTER MAP:
  dev      -> TECH
  research -> ACADEMIC
  career   -> PROFESSIONAL
  work     -> PROFESSIONAL
...
IF score < 0.50:
    REVIEW each flag
    ADJUST vocabulary rarity (use less common alternatives)
    VARY sentence lengths (mix short directives with longer analysis)
    REMOVE em-dashes and Claude-tell vocabulary
    RE-SCORE
```
