...
```
INPUT_ANALYSIS → DETECT context_markers → SELECT voice_mode
...
→ VIOLATION: too formal, no contractions, corporate tone

"That timeline is honestly quite aggressive! Perhaps we should consider building in some buffer time?"
→ VIOLATION: rhetorical question, hedging, exclamation point overuse
```
...
→ VIOLATION: no contractions, too formal, no experiential context, weak connection

"Have you considered implementing request queuing? That might help with the rate limiting issue!"
→ VIOLATION: rhetorical question, exclamation point, no substance
```
...
→ VIOLATIONS: rhetorical questions, weak data, emotional conclusion, hand-holding language, ends with question not fact
...
→ VIOLATIONS: exclamation points, emotional rallying cry, no data, performative outrage
```
...
→ VIOLATIONS: promotional language, manufactured enthusiasm, no specific data, redundant phrasing, applicant-to-gatekeeper tone
...
→ VIOLATIONS: corporate buzzwords, no specific data, promotional tone, vague claims
```
...
7. ASSESS voice_authenticity ≥90%
```
...
TARGET: ≥90 points = authentic voice
```

---

**END VOICE_CALIBRATION_MATRIX_v1.0**
